...
:just make sure we are not in the refractory period	
	...
	count = 0 : initialize dcaap counting
}
...
BEFORE BREAKPOINT {	
	: 1. activate the dcaap after refractory period
	: 2. activate the spike only if threshold was crossed
	: 3. only in compartments where w is set to > 0
	if (t >= t_dCaAP + refract_period && v > vth && w > 0) {
		...
		printf("Fired dCaAP at %f ms with K = %f\n", t_dCaAP,K)
		count = count + 1
	}
	: B is equal to 0 only when spike is initially triggered.
	if(B == 0 && t >= t_dCaAP + sigma_diff){
		B=0.001
	}
